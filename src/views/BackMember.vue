  ...
  <div class="back_content">
    <div id="backMember" class="p-5">
      ...
      >
        ...
        <b-table-column sortable searchable field="email" label="信箱" width="200" v-slot="props">
          <b-input v-model="props.row.model01" v-if="props.row.edit"></b-input>
          <h1 v-else>{{ props.row.email }}</h1>
        </b-table-column>
        <b-table-column sortable searchable field="password" label="密碼" width="150" v-slot="props">
          <b-input v-model="props.row.model02" v-if="props.row.edit"></b-input>
          <h1 v-else>{{ props.row.password }}</h1>
        </b-table-column>
        <b-table-column sortable searchable field="name" label="暱稱" width="150" v-slot="props">
          <b-input v-model="props.row.model03" v-if="props.row.edit"></b-input>
          <div v-else>{{ props.row.name }}</div>
        </b-table-column>
        <b-table-column sortable searchable field="avator" label="頭像" width="150" v-slot="props">
          <b-radio v-model="props.row.model04"
            v-if="props.row.edit"
            name="avator"
            native-value="bear">
            Bear
        ...
        <b-table-column sortable searchable field="authority" label="權限" width="150" v-slot="props">
          <b-radio v-model="props.row.model05"
            v-if="props.row.edit"
            name="authority"
            native-value="true">
            True
      ...
      @click="addModalActive = true">
        <b-icon class="animate__animated animate__pulse animate__infinite animate__slow" icon="account-plus-outline">
        </b-icon>
      </b-button>

      <b-modal :active.sync="addModalActive">
        <form @submit.prevent="onSubmit"  @reset="onReset" class="is-flex is-justify-content-center is-align-items-center">
...
export default {
  ...
  methods: {
    ...
    // 保存
    save (props) {
      this.axios.patch(process.env.VUE_APP_API + '/users/' + props.row._id, { email: props.row.model01, password: props.row.model02, name: props.row.model03, avator: props.row.model04, authority: props.row.model05 })
        .then(res => {
          if (res.data.success) {
            props.row.edit = false
    ...
    del (props) {
      this.axios.delete(process.env.VUE_APP_API + '/users/' + props.row._id)
        .then(res => {
          if (res.data.success) {
            this.$buefy.dialog.alert({
              title: 'Success!',
